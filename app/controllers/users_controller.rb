class UsersController < ApplicationController
  ...
  def following
    ...
    # @following_users = @following_users.order(id: :desc).page(params[:page]).per(2)
  end

  def followers
    @user = User.find(params[:user_id])
    followers_id = Relationship.where(follow_id: params[:user_id]).order(id: :desc).pluck(:user_id)
    @followers = []
